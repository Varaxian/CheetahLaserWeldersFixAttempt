...
namespace Cheetah.Networking
{
    public class AutoSet<T>
    {
        ...
        public void Set(T New)
        {
            ...
            {
                Radars.SessionCore.DebugWrite($"AutoSet[{Entity.DisplayName}]", $"Invalid value supplied: {New.ToString()}");
            }
        }
    ...
    public static class Networker
    {
        ...
        {
            Radars.SessionCore.DebugWrite(Source, Message, DebugPrefix: Prefix);
        }

        static void Report(Exception Scrap, string Source, string Prefix = "Networker.")
        {
            Radars.SessionCore.LogError(Source, Scrap, DebugPrefix: Prefix);
        }
    }
